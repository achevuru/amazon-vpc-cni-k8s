...
func New(useCustomNetworking, usePrefixDelegation, v4Enabled, v6Enabled bool) (*EC2InstanceMetadataCache, error) {
	...
	cache.usePrefixDelegation = usePrefixDelegation
...
func (cache *EC2InstanceMetadataCache) getENIMetadata(eniMAC string) (ENIMetadata, error) {
	...
	start := time.Now()
	log.Debugf("Querying for prefix(es)")
	...
	err = retry.NWithBackoff(retry.NewSimpleBackoff(time.Millisecond*100, maxBackoffDelay, 0.15, 2.0), maxENIEC2APIRetries, func() error {
		...
		for _, returnedENI := range enis {
			if eni == returnedENI.ENIID {
				...
				if (eniIPCount > wantedSecondaryIPsOrPrefixes && !cache.usePrefixDelegation) ||
					(eniIPCount >= wantedSecondaryIPsOrPrefixes && cache.usePrefixDelegation) {
					return nil
				}
...
// GetLocalIPv4 returns the primary IPv4 address on the primary interface
