...
func (s *server) DelNetwork(ctx context.Context, in *rpc.DelNetworkRequest) (*rpc.DelNetworkReply, error) {
	...
	if eni != nil && eni.AvailableIPv4Cidrs[cidrStr] != nil {
		if s.ipamContext.enablePrefixDelegation && eni.AvailableIPv4Cidrs[cidrStr].IsPrefix == false {
			log.Debugf("IP belongs to secondary pool with PD enabled so free IP from EC2")
			s.ipamContext.tryUnassignIPFromENI(eni.ID)
		} else if !s.ipamContext.enablePrefixDelegation && eni.AvailableIPv4Cidrs[cidrStr].IsPrefix == true {
...
func (s *server) mustEmbedUnimplementedCNIBackendServer() {
