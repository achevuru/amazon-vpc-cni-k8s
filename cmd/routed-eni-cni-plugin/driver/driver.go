...
func (createVethContext *createVethPairContext) run(hostNS ns.NetNS) error {
	...
	// Add a connected route to a dummy next hop. v4 - 169.254.1.1; v6 - fe80::1
	// # ip route show
	// default via 169.254.1.1 dev eth0 (or) default via fe80::1 dev eth0
	// 169.254.1.1 dev eth0 (or) fe80::1 dev eth0
	var gwIP string
	var maskLen int
	...
	//TODO - How about v6? Default Neighbor MAC entry?
	neigh := &netlink.Neigh{
		LinkIndex:    contVeth.Attrs().Index,
...
func (os *linuxNetwork) SetupNS(hostVethName string, contVethName string, netnsPath string, v4Addr *net.IPNet, v6Addr *net.IPNet, deviceNumber int, vpcCIDRs []string, useExternalSNAT bool, mtu int, log logger.Logger) error {
